...
/**
 *  This page retreives and shows the planet details.
 */
  ...
  useEffect(() => {
    if (store.planet.planetExists(location.state.id)) {
      ...
      request(
        ...
        .catch((error) =>
          console.log("There was a problem requesting data.", error)
        );
    }
  }, []);

  const handleSavePlanetClick = () => {
    // Can only delete if no edit forms are open.
    if (planetSaved && !showDescriptionForm && !showModal) {
      if (
        window.confirm(
          "Are you sure you want to remove this planet? \nAll edited information will be lost!"
        )
      ) {
      ...
      setPlanetSaved(true);
    } else if (showDescriptionForm || showModal) {
      alert(
        "Please close any open forms before attempting to remove a planet."
      );
    }
  };
      ...
      <div className="w-full flex flex-wrap justify-center">
        <div className="px-2 lg:w-1/2 md:w-3/4 flex flex-col">
          <h1
            ...
            } text-4xl font-bold text-amber-400 flex justify-center animate-ping`}
          >
            Loading
          </h1>
          <div
            id="planet-details"
            className={`${loaded ? "visible" : "hidden"} text-gray-50`}
          >
            ...
            <div>
              ...
              <DescriptionInput
                ...
                currentDescription={
                  store.planet.getPlanet(location.state.id).description
                }
                handleSave={saveDescription}
              />
              <p className={showDescriptionForm ? "hidden" : "visible"}>
                {store.planet.getSelectedPlanet().description ||
                  "No description."}
              </p>
            </div>
            <div>
            ...
            <div
              id="resident-add"
              className={planetSaved ? "visible" : "hidden"}
            >
              <InputModal
                show={showModal}
                handleClose={() => setShowModal(false)}
                planetID={location.state.id}
                handleSave={saveResident}
              />
              <button
                type="button"
                onClick={showModalHandler}
                className="text-amber-400 my-2 font-bold p-2 border border-amber-400 rounded hover:bg-amber-400 hover:text-gray-50"
              >
                Add Resident
              </button>
            </div>
